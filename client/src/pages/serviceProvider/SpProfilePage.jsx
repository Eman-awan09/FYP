...
const SpProfilePage = () => {
  ...
  /* Track whether the profile has been loaded for the first time.
     After that, saveProfile updating `profile` must NOT reset the form. */
  ...
  const handleSubmit = async (e) => {
    ...
    if (!result.success) {
      ...
      /* Sync formValues with whatever the server actually saved,
         but do it explicitly here — not through the useEffect.    */
    ...
    return (
      <div className="sp-page sp-page--center">
        <div className="sp-status-card">
          <h2>No profile data found.</h2>
          <p>
            Please contact administration to set up your service provider
            account.
          </p>
        </div>
      </div>
  ...
  const specialization =
    formValues.specialization || profile.specialization || "";
  const campusOrBuilding =
    formValues.campusOrBuilding || profile.campusOrBuilding || "";
  const role = profile.role || "SERVICE_PROVIDER";
  const initial = (displayName || "S").trim().charAt(0).toUpperCase();
